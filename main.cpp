...
Image* image1 = loadBMP("textures/pattern7.bmp");//normal
Image* image2 = loadBMP("textures/pattern5.bmp");//direction
Image* image3 = loadBMP("textures/pattern9.bmp");//slow
Image* image4 = loadBMP("textures/pattern10.bmp");//fast
void drawFirstCube();
void drawSecondCube();
void drawThirdCube();
void drawAddlevel2();
...
bool win = false;
bool gameOver = false; int timer = 100000;

//sndPlaySound("paradise2.wav",SND_ASYNC)
...
/*void Init()
{
    SoundEngine->play2D("paradise2.wav", GL_TRUE);
}*/
...
if(color_effect_check != color_effect){
	...
	if (!win&&!gameOver){
		...
		glDepthMask(0);
		char buffer[255];
		sprintf_s(buffer, "%s", "Game Over : bara ya yad !"); //%d is for integers 
		renderBitmapString(5.0f, 0.0f, GLUT_BITMAP_HELVETICA_18, buffer);
...
///////////////////////////////////////////////////////////////////////////////////end  of asgher cube (done) sohaaa
...
eyeZ += 0.8; //deadline was 1.2
}


}
else if (eyeZ>3.2){ //tht  //was 3.8 -->d
eyeZ -= 0.7;

}

if (eyeX<-4){//left wall it was 4.7  -->d deadline <-4.3
eyeX += 0.7;
}
else if (eyeX>4.2){//right wall/it was  4.98  -->d
eyeX -= 0.7;
}
else if (eyeX <3.8 && eyeX > -3.1 && eyeZ<3.9&&eyeZ>2.6){//kan tht wi hab ytl3 --->d
///-3.3
printf_s("%s", "   hhh  ");
if (eyeZ <= 2.8){ //it was 3 
...
else if (eyeX>-3.5 && ((eyeZ > -2.3&&eyeZ<0.7) || (eyeZ>1.7&&eyeZ < 2.7))){//mashy ymeen wi hakhbt fel sogher //eyex>3.2 -->d
//deadline was >-3.9i                                  //2.98
//left up 
eyeX -= 0.3;  //it was 0.7
}
...
else if (eyeX <8.1 && eyeX > -7.5 && eyeZ<8.5 && eyeZ>6.5){//kan tht wi hab ytl3s fmmkn ykhbt fel soghnn
///deadline was eyex<8.3
...
int main(int argc, char** argv) {
	...
	Image* image6= loadBMP("textures/pattern9.bmp");
	_textureId3= loadTexture(image6);
    
	PlaySoundA("paradise2.wav",NULL,SND_ASYNC);

	//alutInit(0, NULL);
	//Init();
	//play2D("paradise2.wav"); 
